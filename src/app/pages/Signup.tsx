...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!acceptedTerms) { setError("You must accept the Terms & Conditions and Privacy Policy"); return; }
    const pwErr = validatePassword(password);
    if (pwErr) { setError(pwErr); return; }
    if (password !== confirmPassword) { setError(t("auth.signup.mismatch")); return; }
    setLoading(true);
        ...
        <div className="w-full max-w-md">
          <div className="bg-[#0a2a3a]/80 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-12 border border-cyan-500/20 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h1 className="text-3xl font-bold text-white mb-2 text-center">{t("auth.signup.title")}</h1>
              <p className="text-gray-400 text-center mb-8 text-sm">{t("auth.signup.subtitle")}</p>

              <button onClick={() => triggerGoogleSignup()}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label htmlFor="name" className="block text-sm text-gray-300 mb-1.5">{t("auth.name")}</label>
                ...
                <div>
                  <label htmlFor="confirmPassword" className="block text-sm text-gray-300 mb-1.5">{t("auth.confirmPassword")}</label>
                  <div className="relative">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <label htmlFor="terms" className="text-sm text-gray-300 cursor-pointer">
                    I agree to the{" "}
                    <Link to="/privacy-policy" target="_blank" className="text-cyan-400 hover:underline">Privacy Policy</Link>
                    {" "}and{" "}
                    <Link to="/cookie-policy" target="_blank" className="text-cyan-400 hover:underline">Cookie Policy</Link>
                  </label>
                </div>
              ...
              <p className="text-center text-gray-400 text-sm mt-6">
                ...
                <Link to="/login" className="text-cyan-400 hover:underline">{t("auth.signup.loginLink")}</Link>
              </p>
            </div>
          </div>
        </div>
